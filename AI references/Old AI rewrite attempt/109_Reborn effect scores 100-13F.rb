class PokeBattle_Battle
  ...
  def pbGetMoveScoreFunctions(move,attacker,opponent,skill,roughdamage,initialscores,scoreindex,
    ...
    score = __e__pbGetMoveScoreFunctions(move,attacker,opponent,skill,roughdamage,initialscores,scoreindex,
                                            score, oppitemworks, attitemworks, aimem, bettertype, roles, tempdam)
    case move.function
      when 0x100 # Rain Dance
        ...
        if @opponent.is_a?(Array) == false
          if (@opponent.trainertype==PBTrainers::SHELLY || @opponent.trainertype==PBTrainers::BENNETTLAURA) &&
      ...
      when 0x118 # Gravity
        ...
        if @field.effects[PBEffects::Gravity]>0
          ...
          if (attitemworks && attacker.item == PBItems::AMPLIFIELDROCK) || $fefieldeffect==37   # Psychic Terrain
            score*=1.5
          end
          psyvar=false
          poisonvar=false
