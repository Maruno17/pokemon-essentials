...
class SafariDataBox < SpriteWrapper
  ...
  def refresh
    ...
    textpos.push([displayBattleOddsInfo(),30,38,false,base,shadow])
  ...
  def displayBattleOddsInfo()
    return _INTL("Catch: {1}% Flee: {2}%",@battle.catchChances,@battle.fleeChances)
  end
...
class PokeBattle_SafariZone
  ...
  attr_accessor :catchChances
  attr_accessor :fleeChances
    ...
    begin
      ...
      loop do
        ...
        when 0   # Ball
          ...
          @ballCount -= 1
          @scene.pbRefresh
          @catchChances = (catchFactor*1275)/100
          if safariBall
