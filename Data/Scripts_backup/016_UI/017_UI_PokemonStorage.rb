...
class PokemonBoxIcon < IconSprite
  ...
  def useRegularIcon(species)
    ...
    bitmapFileName = sprintf("Graphics/Icons/icon%03d", species)
  ...
  def createFusionIcon(species)
    bodyPoke = getBodyID(species)
    headPoke = getHeadID(species, bodyPoke)

    icon1 = AnimatedBitmap.new(pbCheckPokemonIconFiles(headPoke))
    icon2 = AnimatedBitmap.new(pbCheckPokemonIconFiles(bodyPoke))

    for i in 0..icon1.width
      for j in (icon1.height / 2) + 10..icon1.height
  ...
  def refresh
    return if !@pokemon
    if useRegularIcon(@pokemon.species) || pokemon.isEgg?
      self.setBitmap(pbPokemonIconFile(pokemon))
    else
      self.bitmap = createFusionIcon(@pokemon.species).bitmap
    end
