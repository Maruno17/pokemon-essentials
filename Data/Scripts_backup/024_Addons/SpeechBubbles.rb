...
def Kernel.pbShowCommands(msgwindow,commands=nil,cmdIfCancel=0,defaultCmd=0)
  defaultCmd = pbGet(249)
  ret=0
  if commands
    if defaultCmd > commands.length
      defaultCmd = 0
      pbSet(249,0)
    end
...
def Kernel.pbMessageNoSound(message,commands=nil,cmdIfCancel=0,skin=nil,defaultCmd=0,&block)
  ret=0
  ...
  if commands
    ...
    Kernel.pbMessageDisplayNoSound(msgwindow,message,&block)
  end
...
def Kernel.pbCreateMessageWindow(viewport=nil,skin=nil)
  if $Bubble==2 # Message window set to floating bubble.
    if $game_player.direction==8 && !$modifDir # Player facing up, message window top. 
...
def getSkinColor(windowskin,color,isDarkSkin)
  if !windowskin || windowskin.disposed? || 
     ...
     textcolors=[
      shadowc3tag(Color.new(72,80,88), MessageConfig::BUBBLETEXTSHADOW),
       "<c2=7E105D08>",
       "<c2=421F2117>",
       "<c2=43F022E8>",
       "<c2=7FF05EE8>",
  ...
  if $game_temp && $game_temp.in_battle && !$scene.respond_to?("update_basic")
    ...
    case $game_system.message_position
      ...
      when 2
        ...
        @NegativeValue = 0
       if $Bubble==1
         msgwindow.setSkin("Graphics/Windowskins/speech pl 20")
         ...
         if $game_player.direction==8 && !$modifDir
           @Restriction = Viewport.new(0, 0, Graphics.width, 280)
           msgwindow.y = 6
         else
...
def pbDisplayGoldWindow(msgwindow)
  if ($game_switches[176])
    moneyString  = $game_variables[50]#.to_str
    goldwindow=Window_AdvancedTextPokemon.new(_INTL("Trainer Points:\n<ar>{1}</ar>",moneyString))
  ...
  #Kernel.pbMessage(_INTL("{1}",msgwindow.y))
  if (msgwindow.y==0 || msgwindow.y==6)
...
####
def pbDisplayTPWindow(msgwindow)
  moneyString= $game_variables[50].to_str
  goldwindow=Window_AdvancedTextPokemon.new(_INTL("Trainer Points:\n<ar>{1}</ar>",moneyString))
  goldwindow.setSkin("Graphics/Windowskins/goldskin")
  goldwindow.resizeToFit(goldwindow.text,Graphics.width)
...
def Kernel.pbMessageDisplayNoSound(msgwindow,message,letterbyletter=true,commandProc=nil)
  Kernel.pbMessageDisplay(msgwindow,message,letterbyletter,commandProc,false)
end

def Kernel.pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil,withSound=true)
  return if !msgwindow
  ...
  for i in 0...controls.length
    ...
    if control=="f"
      ...
      facewindow=PictureWindow.new("Graphics/Battlers/#{param}")
    elsif control=="fff"
      facewindow.dispose if facewindow
      facewindow=PictureWindow.new("Graphics/Battlers/#{param}")      
    elsif control=="op"
    ...
    for i in 0...controls.length
      if controls[i] && controls[i][2]<=msgwindow.position && msgwindow.waitcount==0
        ...
        if control=="f"
          ...
          #
          facewindow.dispose if facewindow
          facewindow= isFusion ? PictureWindow.new("Graphics/Battlers/#{head}/#{head}.#{body}") : PictureWindow.new("Graphics/Battlers/#{head}/#{head}.png")
          pbPositionNearMsgWindow(facewindow,msgwindow,:left)
          facewindow.viewport=msgwindow.viewport
          facewindow.z=msgwindow.z
        elsif control=="fff"
        ...
        elsif control=="tp" # Display TP window
          goldwindow.dispose if goldwindow
          goldwindow=pbDisplayTPWindow(msgwindow)
