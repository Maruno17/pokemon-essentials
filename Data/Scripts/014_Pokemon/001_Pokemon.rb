#===============================================================================
# Instances of this class are individual Pokemon.
# The player's party Pokemon are stored in the array $Trainer.party.
#===============================================================================
class Pokemon
  # @return [Symbol] this Pokemon's species
  attr_reader   :species
  # If defined, this Pokemon's form will be this value even if a MultipleForms
  # handler tries to say otherwise.
  # @return [Integer, nil] this Pokemon's form
  attr_accessor :forced_form
  # If defined, is the time (in Integer form) when this Pokemon's form was set.
  # @return [Integer, nil] the time this Pokemon's form was set
  attr_accessor :time_form_set
  # @return [Integer] the current experience points
  attr_reader   :exp
  # @return [Integer] the number of steps until this Pokemon hatches, 0 if this Pokemon is not an egg
  attr_accessor :steps_to_hatch
  # @return [Integer] the current HP
  attr_reader   :hp
  # @return [Symbol] this Pokemon's current status (see GameData::Status)
  attr_reader   :status
  ...
  # This Pokemon's shininess (true, false, nil). Is recalculated if made nil.
  # @param value [Boolean, nil] whether this Pokemon is shiny
  attr_writer   :shiny
  # The index of this Pokemon's ability (0, 1 are natural abilities, 2+ are
  ...
  # @return [Array<Pokemon::Move>] the moves known by this Pokemon
  attr_accessor :moves
  # @return [Array<Integer>] the IDs of moves known by this Pokemon when it was obtained
  attr_accessor :first_moves
  # @return [Array<Symbol>] an array of ribbons owned by this Pokemon
  ...
  # @return [Integer] this Pokemon's current happiness (an integer between 0 and 255)
  attr_accessor :happiness
  # @return [Symbol] the item ID of the Poké Ball this Pokemon is in
  attr_accessor :poke_ball
  # @return [Integer] this Pokemon's markings, one bit per marking
  ...
  # @return [Hash<Integer>] this Pokemon's effort values
  attr_accessor :ev
  # @return [Integer] calculated stats
  attr_reader   :totalhp, :attack, :defense, :spatk, :spdef, :speed
  # @return [Owner] this Pokemon's owner
  attr_reader   :owner
  # @return [Integer] the manner this Pokemon was obtained:
  #   0 (met), 1 (as egg), 2 (traded), 4 (fateful encounter)
  attr_accessor :obtain_method
  # @return [Integer] the ID of the map this Pokemon was obtained in
  ...
  # @return [Integer] the level of this Pokemon when it was obtained
  ...
  # @return [Pokemon, nil] the Pokemon fused into this one (nil if there is none)
  attr_accessor :fused
  # @return [Integer] this Pokemon's personal ID
  ...
  # Changes the Pokemon's species and re-calculates its statistics.
  # @param species_id [Integer] id of the species to change this Pokemon to
  ...
  # @return [Boolean] whether this Pokemon is of the specified species
  ...
  # @return [Integer] this Pokemon's level
  def level
    @level = growth_rate.level_from_exp(@exp) if !@level
    return @level
  end

  # Sets this Pokemon's level. The given level must be between 1 and the
  ...
  # Sets this Pokemon's Exp. Points.
  ...
  # @return [Boolean] whether this Pokemon is an egg
  def egg?
    return @steps_to_hatch > 0
  end

  # @return [GameData::GrowthRate] this Pokemon's growth rate
  def growth_rate
    return GameData::GrowthRate.get(species_data.growth_rate)
  end

  # @return [Integer] this Pokemon's base Experience value
  def base_exp
  ...
  # Sets this Pokemon's status. See {GameData::Status} for all possible status effects.
  # @param value [Integer, Symbol, String] status to set
  def status=(value)
    return if !able?
    new_status = GameData::Status.try_get(value)
    if !new_status
      raise ArgumentError, _INTL('Attempted to set {1} as Pokemon status', value.class.name)
    end
    @status = new_status.id
  end

  # @return [Boolean] whether the Pokemon is not fainted and not an egg
  def able?
    return !egg? && @hp > 0
  end

  # @return [Boolean] whether the Pokemon is fainted
  def fainted?
    return !egg? && @hp <= 0
  end
  ...
  # Restores all PP of this Pokemon. If a move index is given, restores the PP
  ...
  # @return [Symbol] this Pokemon's first type
  def type1
    return species_data.type1
  end

  # @return [Symbol] this Pokemon's second type, or the first type if none is defined
  def type2
    sp_data = species_data
    return sp_data.type2 || sp_data.type1
  end

  # @return [Array<Symbol>] an array of this Pokemon's types
  def types
    sp_data = species_data
    ret = [sp_data.type1]
  ...
  # @return [Boolean] whether this Pokemon has the specified type
  def hasType?(type)
  ...
  # @return [0, 1, 2] this Pokemon's gender (0 = male, 1 = female, 2 = genderless)
  def gender
    if !@gender
  ...
  # Makes this Pokemon male.
  def makeMale; self.gender = 0; end

  # Makes this Pokemon female.
  def makeFemale; self.gender = 1; end

  # @return [Boolean] whether this Pokemon is male
  def male?; return self.gender == 0; end

  # @return [Boolean] whether this Pokemon is female
  def female?; return self.gender == 1; end

  # @return [Boolean] whether this Pokemon is genderless
  def genderless?; return self.gender == 2; end

  # @return [Boolean] whether this Pokemon species is restricted to only ever being one
  ...
  # @return [Boolean] whether this Pokemon is shiny (differently colored)
  def shiny?
    if @shiny.nil?
  ...
  # @return [Integer] the index of this Pokemon's ability
  def ability_index
    @ability_index = (@personalID & 1) if !@ability_index
    return @ability_index
  end

  # @return [GameData::Ability, nil] an Ability object corresponding to this Pokemon's ability
  def ability
    return GameData::Ability.try_get(ability_id)
  end

  # @return [Symbol, nil] the ability symbol of this Pokemon's ability
  def ability_id
    if !@ability
      sp_data = species_data
  ...
  # Returns whether this Pokemon has a particular ability. If no value
  # is given, returns whether this Pokemon has an ability set.
  # @param check_ability [Symbol, GameData::Ability, Integer, nil] ability ID to check
  # @return [Boolean] whether this Pokemon has a particular ability or
  #   an ability at all
  ...
  # @return [Boolean] whether this Pokemon has a hidden ability
  def hasHiddenAbility?
    return ability_index >= 2
  end

  # @return [Array<Array<Symbol,Integer>>] the abilities this Pokemon can have,
  ...
  # @return [GameData::Nature, nil] a Nature object corresponding to this Pokemon's nature
  def nature
  ...
  # @return [GameData::Nature, nil] this Pokemon's calculated nature
  ...
  # If defined, this Pokemon's nature is considered to be this when calculating stats.
  ...
  # Returns whether this Pokemon has a particular nature. If no value is given,
  # returns whether this Pokemon has a nature set.
  # @param check_nature [Integer] nature ID to check
  # @return [Boolean] whether this Pokemon has a particular nature or a nature
  #   at all
  ...
  # @return [GameData::Item, nil] an Item object corresponding to this Pokemon's item
  def item
  ...
  # Gives an item to this Pokemon to hold.
  ...
  # Returns whether this Pokemon is holding an item. If an item id is passed,
  # returns whether the Pokemon is holding that item.
  # @param check_item [Symbol, GameData::Item, Integer] item ID to check
  # @return [Boolean] whether the Pokemon is holding the specified item or
  #   an item at all
  ...
  # @return [Mail, nil] mail held by this Pokemon (nil if there is none)
  def mail
    @mail = nil if @mail && (!@mail.item || !hasItem?(@mail.item))
    return @mail
  end

  # If mail is a Mail object, gives that mail to this Pokemon. If nil is given,
  # removes the held mail.
  # @param mail [Mail, nil] mail to be held by this Pokemon
  def mail=(mail)
  ...
  # @return [Integer] the number of moves known by the Pokemon
  def numMoves
    return @moves.length
  end

  # @param move_id [Symbol, String, Integer] ID of the move to check
  # @return [Boolean] whether the Pokemon knows the given move
  def hasMove?(move_id)
  ...
  # Returns the list of moves this Pokemon can learn by levelling up.
  # @return [Array<Array<Integer,Symbol>>] this Pokemon's move list, where every element is [level, move ID]
  def getMoveList
    return species_data.moves
  end

  # Sets this Pokemon's movelist to the default movelist it originally had.
  def reset_moves
  ...
  # Deletes the given move from the Pokemon.
  ...
  # Adds a move to this Pokemon's first moves.
  ...
  # Removes a move from this Pokemon's first moves.
  ...
  # Clears this Pokemon's first moves.
  def clear_first_moves
    @first_moves.clear
  end

  # @param move_id [Symbol, String, Integer] ID of the move to check
  # @return [Boolean] whether the Pokemon is compatible with the given move
  ...
  # Removes the specified ribbon from this Pokemon.
  ...
  # Removes all ribbons from this Pokemon.
  def clearAllRibbons
    @ribbons.clear
  end
  ...
  # @return [Integer] the Pokérus infection stage for this Pokemon
  def pokerusStrain
    return @pokerus / 16
  end
  ...
  def givePokerus(strain = 0)
    return if self.pokerusStage == 2   # Can't re-infect a cured Pokemon
  ...
  # @return [Boolean] whether the given trainer is not this Pokemon's original trainer
  def foreign?(trainer)
    return @owner.id != trainer.id || @owner.name != trainer.name
  end

  # @return [Time] the time when this Pokemon was obtained
  def timeReceived
    return Time.at(@timeReceived)
  end

  # Sets the time when this Pokemon was obtained.
  # @param value [Integer, Time, #to_i] time in seconds since Unix epoch
  def timeReceived=(value)
    @timeReceived = value.to_i
  end

  # @return [Time] the time when this Pokemon hatched
  def timeEggHatched
    return (obtain_method == 1) ? Time.at(@timeEggHatched) : nil
  end

  # Sets the time when this Pokemon hatched.
  # @param value [Integer, Time, #to_i] time in seconds since Unix epoch
  ...
  # @return [String] the name of this Pokemon
  def name
    return (nicknamed?) ? @name : speciesName
  end

  # @param value [String] the nickname of this Pokemon
  def name=(value)
    value = nil if !value || value.empty? || value == speciesName
    @name = value
  end

  # @return [Boolean] whether this Pokemon has been nicknamed
  def nicknamed?
    return @name && !@name.empty?
  end

  # @return [String] the species name of this Pokemon
  def speciesName
    return species_data.name
  end

  # @return [Integer] the height of this Pokemon in decimetres (0.1 metres)
  def height
    return species_data.height
  end

  # @return [Integer] the weight of this Pokemon in hectograms (0.1 kilograms)
  def weight
    return species_data.weight
  end

  # @return [Hash<Integer>] the EV yield of this Pokemon (a hash with six key/value pairs)
  def evYield
  ...
  # @param other_pkmn [Pokemon] the other Pokemon involved in the trade
  # @return [Symbol, nil] the ID of the species to evolve into
  ...
  # Called after this Pokemon evolves, to remove its held item (if the evolution
  # required it to have a held item) or duplicate this Pokemon (Shedinja only).
  # @param new_species [Pokemon] the species that this Pokemon evolved into
  ...
  # @return [Hash<Integer>] this Pokemon's base stats, a hash with six key/value pairs
  def baseStats
  ...
  # Returns this Pokemon's effective IVs, taking into account Hyper Training.
  # Only used for calculating stats.
  # @return [Hash<Integer>] hash containing this Pokemon's effective IVs
  def calcIV
    this_ivs = self.iv
    ret = {}
  ...
  # @return [Integer] the maximum HP of this Pokemon
  def calcHP(base, level, iv, ev)
    return 1 if base == 1   # For Shedinja
    return ((base * 2 + iv + (ev / 4)) * level / 100).floor + level + 10
  end

  # @return [Integer] the specified stat of this Pokemon (not used for total HP)
  ...
  # Creates a copy of this Pokemon and returns it.
  # @return [Pokemon] a copy of this Pokemon
  def clone
    ret = super
    ret.iv          = {}
    ret.ivMaxed     = {}
  ...
  # @param species [Symbol, String, Integer] Pokemon species
  # @param level [Integer] Pokemon level
  # @param owner [Owner, Player, NPCTrainer] Pokemon owner (the player by default)
  # @param withMoves [TrueClass, FalseClass] whether the Pokemon should have moves
