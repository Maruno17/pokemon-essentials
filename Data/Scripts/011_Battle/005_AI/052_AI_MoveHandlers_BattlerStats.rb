...
Battle::AI::Handlers::MoveEffectScore.add("RaiseUserMainStats1LoseThirdOfTotalHP",
  proc { |score, move, user, target, skill, ai, battle|
    ...
    next 0 if user.hasActiveAbility?(:CONTRARY)
    stats_maxed = true
    ...
    if ai.skill_check(Battle::AI::AILevel.high) && user.hp >= user.totalhp * 0.75
      score += 30
    end
    GameData::Stat.each_main_battle { |s| score += 10 if user.stages[s.id] <= 0 }
    if ai.skill_check(Battle::AI::AILevel.medium)
...
Battle::AI::Handlers::MoveEffectScore.add("RaiseTargetAttack2ConfuseTarget",
  proc { |score, move, user, target, skill, ai, battle|
    next score - 90 if !target.pbCanConfuse?(user, false)
...
Battle::AI::Handlers::MoveEffectScore.add("RaiseTargetAtkSpAtk2",
  proc { |score, move, user, target, skill, ai, battle|
    ...
    next score - 90 if ai.skill_check(Battle::AI::AILevel.medium) && target.hasActiveAbility?(:CONTRARY)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetAttack1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:ATTACK, user)
      score += target.stages[:ATTACK] * 20
      if ai.skill_check(Battle::AI::AILevel.medium)
        hasPhysicalAttack = false
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetDefense1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:DEFENSE, user)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetSpAtk1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:SPECIAL_ATTACK, user)
      score += user.stages[:SPECIAL_ATTACK] * 20
      if ai.skill_check(Battle::AI::AILevel.medium)
        hasSpecicalAttack = false
        target.eachMove do |m|
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetSpAtk2IfCanAttract",
  proc { |score, move, user, target, skill, ai, battle|
    next 0 if user.gender == 2 || target.gender == 2 ||
              user.gender == target.gender || target.hasActiveAbility?(:OBLIVIOUS)
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:SPECIAL_ATTACK, user)
      score += 40 if user.turnCount == 0
      score += target.stages[:SPECIAL_ATTACK] * 20
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetSpDef1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:SPECIAL_DEFENSE, user)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetSpeed1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:SPEED, user)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetSpeed1MakeTargetWeakerToFire",
  proc { |score, move, user, target, skill, ai, battle|
    next 0 if !target.pbCanLowerStatStage?(:SPEED, user) && target.effects[PBEffects::TarShot]
    score += target.stages[:SPEED] * 10
    if ai.skill_check(Battle::AI::AILevel.high)
      aspeed = pbRoughStat(user, :SPEED)
      ospeed = pbRoughStat(target, :SPEED)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetAccuracy1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      return 0 if !target.pbCanLowerStatStage?(:ACCURACY, user)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetEvasion1",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      return 0 if !target.pbCanLowerStatStage?(:EVASION, user)
...
Battle::AI::Handlers::MoveEffectScore.add("LowerTargetEvasion1RemoveSideEffects",
  proc { |score, move, user, target, skill, ai, battle|
    if move.statusMove?
      next 0 if !target.pbCanLowerStatStage?(:EVASION, user)
      score += target.stages[:EVASION] * 10
    elsif target.stages[:EVASION] > 0
      score += 20
    end
...
Battle::AI::Handlers::MoveEffectScore.add("RaiseUserAndAlliesAtkDef1",
  proc { |score, move, user, target, skill, ai, battle|
    ...
    user.allAllies.each do |b|
      ...
      has_ally = true
      if ai.skill_check(Battle::AI::AILevel.medium) && b.hasActiveAbility?(:CONTRARY)
        score -= 90
      else
        score += 40
...
Battle::AI::Handlers::MoveEffectScore.add("UserSwapBaseAtkDef",
  proc { |score, move, user, target, skill, ai, battle|
    if ai.skill_check(Battle::AI::AILevel.medium)
      aatk = pbRoughStat(user, :ATTACK)
      adef = pbRoughStat(user, :DEFENSE)
      if aatk == adef ||
...
Battle::AI::Handlers::MoveEffectScore.add("UserTargetAverageBaseAtkSpAtk",
  proc { |score, move, user, target, skill, ai, battle|
    if ai.skill_check(Battle::AI::AILevel.medium)
      aatk   = pbRoughStat(user, :ATTACK)
      aspatk = pbRoughStat(user, :SPECIAL_ATTACK)
      oatk   = pbRoughStat(target, :ATTACK)
      ospatk = pbRoughStat(target, :SPECIAL_ATTACK)
...
Battle::AI::Handlers::MoveEffectScore.add("UserTargetAverageBaseDefSpDef",
  proc { |score, move, user, target, skill, ai, battle|
    if ai.skill_check(Battle::AI::AILevel.medium)
      adef   = pbRoughStat(user, :DEFENSE)
      aspdef = pbRoughStat(user, :SPECIAL_DEFENSE)
      odef   = pbRoughStat(target, :DEFENSE)
      ospdef = pbRoughStat(target, :SPECIAL_DEFENSE)
