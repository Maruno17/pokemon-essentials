...
class BetterRegionMap
  ...
  def initialize(region = -1, show_player = true, can_fly = false, wallmap = false, species = nil)
    showBlk
    ...
    windowminx = -1 * (@window["map"].bmp.width - SCREEN_WIDTH)
    windowminx = 0 if windowminx > 0
    windowminy = -1 * (@window["map"].bmp.height - SCREEN_HEIGHT)
    windowminy = 0 if windowminy > 0
    
    if @sprites["cursor"].x > (SCREEN_WIDTH / 2)
      @window.x = (SCREEN_WIDTH / 2 ) - @sprites["cursor"].x
      if (@window.x < windowminx)
        @window.x = windowminx
      end     
      @sprites["cursor"].x += @window.x
    end
    if @sprites["cursor"].y > (SCREEN_HEIGHT / 2)
      @window.y = (SCREEN_HEIGHT / 2 ) - @sprites["cursor"].y
...
class PokemonPokedexInfo_Scene
  def drawPageArea
    ...
    windowminx = -1 * (@sprites["areamap"].bmp.width - SCREEN_WIDTH + 16)
    windowminy = -1 * (@sprites["areamap"].bmp.height - SCREEN_HEIGHT + 16)
