...
#  Isle of Armor DLC.
#===============================================================================
# You only need to call pbCramOMatic in an event, and the script will allow
...
def pbCramOMatic
  if Kernel.pbConfirmMessage(_INTL("Combine items?"))
    ...
    if items.all?{|item| pbIsApricorn?(item)}
      ...
      for i in 0...CRAMOMATIC_RATIO.length
        if value<=CRAMOMATIC_RATIO[i] || CRAMOMATIC_RATIO[i]==-1
          index=i
          break
        end
      end
