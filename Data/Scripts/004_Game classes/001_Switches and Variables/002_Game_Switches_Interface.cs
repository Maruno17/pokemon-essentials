...
namespace PokemonEssentials
{
	...
	/// </summary>
	/// <remarks>
	/// Refer to <see cref="Game.switches"/> for the instance of this class.
	/// Switches are boolean flags used to track game state and progression. They are saved with the game
	/// and can be used to control event flow, battle conditions, and other game mechanics.
	///
	/// The following is a list of predefined switches and their purposes:
	/// <code>
	...
	/// 14	*s:DayNight.isDay?	Is ON during the day (i.e. between 5am and 8pm), and OFF otherwise (i.e. during the night).
	/// 15	*s:DayNight.isNight?	Is ON during the night (i.e. between 8pm and 5am), and OFF otherwise (i.e. during the day).
	/// 16	*s:DayNight.isMorning?	Is ON during the morning (i.e. between 5am and 10am), and OFF otherwise.
	/// 17	*s:DayNight.isAfternoon?	Is ON during the afternoon (i.e. between 2pm and 5pm), and OFF otherwise.
	/// 18	*s:DayNight.isEvening?	Is ON during the evening (i.e. between 5pm and 8pm), and OFF otherwise.
	/// 19	*s:IsWeekday(-1,2,4,6)	Is ON if the current day is Tuesday, Thursday or Saturday, and OFF otherwise.
	/// 20	*s:!IsWeekday(-1,2,4,6)	The opposite of Switch 19. Is ON if the current day is Monday, Wednesday, Friday or Sunday, and OFF otherwise.
	...
	/// 25	*s:InSafari?	Is ON if the player is currently in the Safari Zone, and OFF if they are not.
	/// 26	*s:BugContestUndecided?	Is ON during a Bug Catching Contest.
	/// 27	*s:BugContestDecided?	Is ON after a Bug Catching Contest has finished. It is used to begin the judging and to show other Contest participants standing around afterwards.
	...
	/// 30	*s:NextMysteryGiftID>0	Is ON if there is a downloaded Mystery Gift which has not yet been collected, and OFF if there are none.
	...
	{
		/// <summary>
		/// Gets or sets the state of a switch.
		/// </summary>
		/// <param name="switch_id">The ID of the switch to get or set.</param>
		/// <value>True if the switch is ON, false if it is OFF.</value>
		/// <returns>The current state of the switch.</returns>
		/// <remarks>
		/// This indexer provides access to individual switches in the game. Each switch is a boolean value
		/// that can be used to track game state and control game flow. The switch_id parameter must be a
		/// valid switch ID (1 or greater).
		///
		/// See the class remarks for a complete list of predefined switches and their purposes.
		/// </remarks>
		/// <exception cref="System.ArgumentOutOfRangeException">Thrown when switch_id is less than 1.</exception>
