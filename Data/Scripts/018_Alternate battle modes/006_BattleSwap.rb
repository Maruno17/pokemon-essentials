class BattleSwapScene
  def pbStartRentScene(rentals)
    ...
    @sprites["title"]=Window_UnformattedTextPokemon.newWithSize(
       _INTL("RENTAL Pokemon"),0,0,Graphics.width,64,@viewport)
  ...
  def pbStartSwapScene(currentPokemon,newPokemon)
    ...
    @viewport.z=99999
    @sprites["title"]=Window_UnformattedTextPokemon.newWithSize(
       _INTL("Pokemon SWAP"),0,0,Graphics.width,64,@viewport)
    @sprites["help"]=Window_UnformattedTextPokemon.newWithSize(
       "",0,Graphics.height-64,Graphics.width,64,@viewport)
    @sprites["list"]=Window_AdvancedCommandPokemonEx.newWithSize(
       [],0,64,Graphics.width,Graphics.height-128,@viewport)
  ...
  def pbInitSwapScreen
    ...
    @sprites["help"].text=_INTL("Select Pokemon to swap.")
  ...
  def pbGetCommands(list, choices)
    ...
    for i in 0...list.length
      ...
      cmd = _INTL("{1} - {2} Pokemon", pkmn.speciesName, category)
  ...
  def pbUpdateChoices(choices)
    ...
    @choices=choices
    if choices.length==0
      @sprites["help"].text=_INTL("Choose the first Pokemon.")
    elsif choices.length==1
      @sprites["help"].text=_INTL("Choose the second Pokemon.")
    else
      @sprites["help"].text=_INTL("Choose the third Pokemon.")
    end
    @sprites["list"].commands=commands
  end
  ...
  def pbSwapChosen(_pkmnindex)
    ...
    commands.push(_INTL("CANCEL"))
    @sprites["help"].text=_INTL("Select Pokemon to accept.")
    @sprites["list"].commands=commands
    @sprites["list"].index=0
    @mode=2
  end
    ...
    loop do
      ...
      if command==0
        ...
        if chosen.any? { |item| item==index }
          ...
          @scene.pbUpdateChoices(chosen.clone)
          if chosen.length==3
            if @scene.pbConfirm(_INTL("Are these three Pokemon OK?"))
              retval=[]
      ...
      if pkmn>=0
        ...
        if command==0
          ...
          loop do
            pkmn=@scene.pbChoosePokemon(true)
            if pkmn>=0
              if @scene.pbConfirm(_INTL("Accept this Pokemon?"))
                @scene.pbEndScene
