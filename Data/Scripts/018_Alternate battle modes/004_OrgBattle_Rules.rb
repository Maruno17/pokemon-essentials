...
class SameSpeciesClause
  ...
  def errorMessage
    return _INTL("Pokemon can't be the same.")
  end
end
...
class PokemonRuleSet
  ...
  def isValid?(team,error=nil)
    if team.length<self.minLength
      error.push(_INTL("Choose a Pokemon.")) if error && self.minLength==1
      error.push(_INTL("{1} Pokemon are needed.",self.minLength)) if error && self.minLength>1
      return false
    elsif team.length>self.maxLength
      error.push(_INTL("No more than {1} Pokemon may enter.",self.maxLength)) if error
      return false
    end
    for pokemon in team
      if !isPokemonValid?(pokemon)
...
the same number of Pokemon left, total HP remaining breaks the tie. If both HP
totals are identical, the battle is a draw.

# Command Time
If the player is in the process of switching Pokemon when the time runs out, the
