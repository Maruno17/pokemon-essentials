        ...
        when 0   # Cure
          ...
          if ids[cmd] == :SLEEP
            ...
            count = pbMessageChooseNumber(
               _INTL("Set the Pokemon's sleep count."), params) { screen.pbUpdate }
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    if pkmn.egg?
      ...
      level = pbMessageChooseNumber(
         _INTL("Set the Pokemon's level (max. {1}).", params.maxNumber), params) { screen.pbUpdate }
      if level != pkmn.level
        pkmn.level = level
        pkmn.calc_stats
        screen.pbRefreshSingle(pkmnid)
    ...
    if pkmn.egg?
      ...
      if minxp == maxxp
        ...
        newexp = pbMessageChooseNumber(
           _INTL("Set the Pokemon's Exp (range {1}-{2}).", minxp, maxxp - 1), params) { screen.pbUpdate }
        if newexp != pkmn.exp
          pkmn.exp = newexp
          pkmn.calc_stats
          screen.pbRefreshSingle(pkmnid)
...
PokemonDebugMenuCommands.register("sethappiness", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    h = pbMessageChooseNumber(
       _INTL("Set the Pokemon's happiness (max. 255)."), params) { screen.pbUpdate }
    if h != pkmn.happiness
      pkmn.happiness = h
      screen.pbRefreshSingle(pkmnid)
    end
...
PokemonDebugMenuCommands.register("setbeauty", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    newval = pbMessageChooseNumber(
       _INTL("Set the Pokemon's Beauty (max. 255)."), params) { screen.pbUpdate }
...
PokemonDebugMenuCommands.register("setcool", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    newval = pbMessageChooseNumber(
       _INTL("Set the Pokemon's Cool (max. 255)."), params) { screen.pbUpdate }
    if newval != pkmn.cool
      pkmn.cool = newval
      screen.pbRefreshSingle(pkmnid)
    end
...
PokemonDebugMenuCommands.register("setcute", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    newval = pbMessageChooseNumber(
       _INTL("Set the Pokemon's Cute (max. 255)."), params) { screen.pbUpdate }
    if newval != pkmn.cute
      pkmn.cute = newval
      screen.pbRefreshSingle(pkmnid)
    end
...
PokemonDebugMenuCommands.register("setsmart", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    newval = pbMessageChooseNumber(
       _INTL("Set the Pokemon's Smart (max. 255)."), params) { screen.pbUpdate }
...
PokemonDebugMenuCommands.register("settough", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    newval = pbMessageChooseNumber(
       _INTL("Set the Pokemon's Tough (max. 255)."), params) { screen.pbUpdate }
...
PokemonDebugMenuCommands.register("setsheen", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    ...
    newval = pbMessageChooseNumber(
       _INTL("Set the Pokemon's Sheen (max. 255)."), params) { screen.pbUpdate }
    ...
    loop do
      ...
      when 1   # Set form
        ...
        if formcmds[0].length <= 1
          ...
          cmd2 = screen.pbShowCommands(_INTL("Set the Pokemon's form."), formcmds[1], cmd2)
          next if cmd2 < 0
          f = formcmds[0][cmd2]
          if f != pkmn.form
...
PokemonDebugMenuCommands.register("shadowpkmn", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    cmd = 0
    loop do
      msg = [_INTL("Not a Shadow Pokemon."),
      ...
      when 0   # Make Shadow
        if !pkmn.shadowPokemon?
          ...
          screen.pbDisplay(_INTL("{1} is already a Shadow Pokemon.", pkmn.name))
        end
      when 1   # Set heart gauge
        if pkmn.shadowPokemon?
          ...
          screen.pbDisplay(_INTL("{1} is not a Shadow Pokemon.", pkmn.name))
        end
      end
    end
    next false
...
PokemonDebugMenuCommands.register("duplicate", {
  ...
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    if screen.pbConfirm(_INTL("Are you sure you want to copy this Pokemon?"))
      clonedpkmn = pkmn.clone
      if screen.is_a?(PokemonPartyScreen)
        pbStorePokemon(clonedpkmn)
        screen.pbHardRefresh
        screen.pbDisplay(_INTL("The Pokemon was duplicated."))
      elsif screen.is_a?(PokemonStorageScreen)
        if screen.storage.pbMoveCaughtToParty(clonedpkmn)
          if pkmnid[0] != -1
            screen.pbDisplay(_INTL("The duplicated Pokemon was moved to your party."))
          end
        else
          ...
          if newbox < 0
            ...
            screen.pbDisplay(_INTL("The duplicated Pokemon was moved to box \"{1}.\"", screen.storage[newbox].name))
...
PokemonDebugMenuCommands.register("delete", {
  ...
  "name"        => _INTL("Delete"),
  "effect"      => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
    if screen.pbConfirm(_INTL("Are you sure you want to delete this Pokemon?"))
      if screen.is_a?(PokemonPartyScreen)
