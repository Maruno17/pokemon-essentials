...
class UIControls::TextBox < UIControls::BaseControl
  TEXT_BOX_X       = 0
  TEXT_BOX_HEIGHT  = 20
  TEXT_BOX_PADDING = 4   # Interior gap between sides of text box and text
  ...
  def reset_display_pos
    inside_box_width = @text_box_rect.width - (TEXT_BOX_PADDING * 2)
    char_widths = []
    @value.to_s.length.times { |i| char_widths[i] = self.bitmap.text_size(@value.to_s[i]).width }
    # Text isn't wider than the box
    if char_widths.sum <= inside_box_width
    ...
    if @cursor_pos > @display_pos
      loop do
        ...
        break if cursor_x < inside_box_width
        @display_pos += 1
        display_pos_changed = true
  ...
  def draw_cursor(cursor_x)
    ...
    bitmap.fill_rect(cursor_x, cursor_y_offset, 2, cursor_height, get_color_of(:text))
  end
  ...
  def refresh
    ...
    if last_char_index < @value.to_s.length - 1
      bitmap.fill_rect(@text_box_rect.x + @text_box_rect.width - 1, (height / 2) - 4, 1, 8, get_color_of(:background))
      5.times do |i|
