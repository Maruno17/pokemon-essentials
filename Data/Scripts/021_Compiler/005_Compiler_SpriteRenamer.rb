...
module Compiler
  ...
  def convert_pokemon_filename(full_name, default_prefix = "")
    ...
    if name[/000/]
      ...
      raise _INTL("Species {1} is not defined (trying to rename Pokemon graphic {2}).", species_number, full_name) if !species_data
  ...
  def convert_pokemon_sprites(src_dir, dest_dir)
    ...
    files.each_with_index do |file, i|
      ...
      pbSetWindowText(_INTL("Converting Pokemon sprites {1}/{2}...", i, files.length)) if i % 50 == 0
      case file
      when "battlers.png"
  ...
  def convert_pokemon_icons(src_dir, dest_dir)
    ...
    files.each_with_index do |file, i|
      Graphics.update if i % 100 == 0
      pbSetWindowText(_INTL("Converting Pokemon icons {1}/{2}...", i, files.length)) if i % 50 == 0
      case file
      when "iconEgg.png"
  ...
  def convert_pokemon_cries(src_dir)
    ...
    files.each_with_index do |file, i|
      Graphics.update if i % 100 == 0
      pbSetWindowText(_INTL("Converting Pokemon cries {1}/{2}...", i, files.length)) if i % 50 == 0
      if file[/^(\d{3})Cry[^\.]*\.([^\.]*)$/]
  ...
  def convert_files
    #return if !pbConfirmMessage("Check for Pokemon/item/trainer files that need renaming?")
    # Rename and move Pokemon sprites/icons
